...
def connection_thread():
    ...
    while True:
        ...
        if msg_from_server == 'move':
            my_move = True
        elif msg_from_server[0] == 'p' and menu == 2:
            # decode data
            print(msg_from_server)
            ...
            for x in range(3):
                for y in range(3):
                    board[x][y] = received_data[b_index + 1 + 3*x + y]
...
def click_board(x, y):
    global my_move
    if my_move:
        print('sending: move {} {}'.format(x, y).encode('utf8'))
...
while True:
    ...
    if menu == 2:
        ...
        for x in range(3):
            for y in range(3):
                gui.add('button_board_{}_{}'.format(x, y), mygui.Button(window, (20 + 20*x, 20 + 20*y), (20, 20), on_action=click_board, on_action_args=(x, y)))
        gui.add('textlist_players', mygui.TextList(window, (80, 20), (100,20), ['xxx']))
